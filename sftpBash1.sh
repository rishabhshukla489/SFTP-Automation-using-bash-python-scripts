...
read_file_content_sftp() {
    ...
    sshpass -p "$password" scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null "$username@$host:$remote_file_path" "$local_file_path" 2>/dev/null
    if [ "$?" -ne 0 ]; then
        echo "Error: Connection disconnect or server connection issue occurred during SFTP read operation."
        exit 1
    fi
    cat "$local_file_path"
    rm "$local_file_path"
...
send_file_content_sftp() {
    ...
    sshpass -p "$password" scp -q -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null "$local_file_path" "$username@$host:$remote_file_path" >/dev/null 2>&1
    if [ "$?" -ne 0 ]; then
        echo "Error: Connection disconnect or server connection issue occurred during SFTP send operation."
        exit 1
    fi
}
...
read_file_content_sftp_key() {
    ...
    local local_file_path="temp_file.txt"
    sshpass -p "$password" scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -i "$key_path" "$username@$host:$remote_file_path" "$local_file_path" 2>/dev/null
    if [ "$?" -ne 0 ]; then
        echo "Error: Connection disconnect or server connection issue occurred during SFTP read operation with key authentication."
        exit 1
    fi
    cat "$local_file_path"
    rm "$local_file_path"
...
send_file_content_sftp_key() {
    ...
    sshpass -p "$password" scp -q -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -i "$key_path" "$local_file_path" "$username@$host:$remote_file_path" >/dev/null 2>&1
    if [ "$?" -ne 0 ]; then
        echo "Error: Connection disconnect or server connection issue occurred during SFTP send operation with key authentication."
        exit 1
    fi
}
...
read_file_content_ftp() {
    ...
    if [ "$?" -ne 0 ]; then
        echo "Error: Connection disconnect or server connection issue occurred during FTP read operation."
        exit 1
    fi
}
...
send_file_content_ftp() {
    ...
    if [ "$?" -ne 0 ]; then
        echo "Error: Connection disconnect or server connection issue occurred during FTP send operation."
        exit 1
    fi
}
